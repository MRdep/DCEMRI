...
;        printf, lun, 'splog, "*-*-*-*-*-*-*-*-*-*"'
    ...
    if n_elements(debug) EQ 0 then begin
        obridge=obj_new("IDL_IDLBridge", output= FILE_EXPAND_PATH('log.log'))

        if nsplit GT 1 then begin
            for i=1, nsplit-1 do begin
                obridge=[obridge, obj_new("IDL_IDLBridge",output= FILE_EXPAND_PATH('log' + strtrim(string(i),1) + '.log'))]
            endfor
...
if keyword_set(wait_interval) then wait, wait_interval
           obridge[i]->IDL_IDLBridge::execute, "cd, fluffx123123pwd"
           ;
           ;obridge[i]->IDL_IDLBridge::execute, "cd, C:\Users\ben dickie\IDLWorkspace82\Default"
           obridge[i]->setvar, varnames[j], (scope_varfetch(varnames[j],level=-1+levoff))
         
        
           
                 endfor
;                   print, obridge[i] ->getvar(varnames[0])
;                   print, obridge[i] ->getvar(varnames[1])
;                   print, obridge[i] ->getvar(varnames[2])
;                   print, obridge[i] ->getvar(varnames[3])
         ...
         ; print, '9',SCOPE_LEVEL()
            ...
            oBridge[i]->IDL_IDLBridge::execute, '@' + 'p'+rstring(i)+basename,/nowait
            IF(~OBJ_VALID(oBridge[i]))THEN BEGIN
              void=DIALOG_MESSAGE(/ERROR,'Unable to access ' $
                + 'an IDL_IDLBridge object')
                
                ENDIF
            ;print, obridge[i] ->status()
        endfor
