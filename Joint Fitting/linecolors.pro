;Viewing contents of file '../idllib/ssw/allpro/linecolors.pro'
;+
;
; file LINECOLORS.PRO
;
...
pro linecolors, mycol_r=mycol_r, mycol_g=mycol_g, mycol_b=mycol_b, $
  ...
  noload=noload, notop=notop, squeeze=squeeze, used=used, $
  ...
  if ((!d.name eq 'X') and (!d.window eq -1)) or  $   ; need a window, or
  ...
  ;
  ; The distinct colors are placed at the bottom of the table
  ;
  
  r_temp(0) = distinct_r  &  g_temp(0) = distinct_g  &  b_temp(0) = distinct_b
  ...
  if (size(mycol_r))(0) eq 0 then begin   ; no colors passed by user
    if not keyword_set(nosqueeze) then $ ; if allowed to squeeze
      r_temp(used) = congrid(reform(r_orig, n_orig, 1), remain, 1)
  endif else $          ; fit in user's colors
    r_temp(used) = congrid(reform(mycol_r, (size(mycol_r))(1), 1), remain, 1)
    
  if (size(mycol_g))(0) eq 0 then begin   ; no colors passed by user
    if not keyword_set(nosqueeze) then $ ; if allowed to squeeze
      g_temp(used) = congrid(reform(g_orig, n_orig, 1), remain, 1)
  endif else $          ; fit in user's colors
    g_temp(used) = congrid(reform(mycol_g, (size(mycol_g))(1), 1), remain, 1)
    
  if (size(mycol_b))(0) eq 0 then begin   ; no colors passed by user
    if not keyword_set(nosqueeze) then $ ; if allowed to squeeze
      b_temp(used) = congrid(reform(b_orig, n_orig, 1), remain, 1)
  endif else $                ; fit in user's colors
    b_temp(used) = congrid(reform(mycol_b, (size(mycol_b))(1), 1), remain, 1)
  ...
  ;
  ; If not 'X' then tell the user what they have just done.
  ;
  
  if !d.name ne 'X' then message, /info, strtrim(!d.table_size,2) + $
    ' color table indices set for ' + !d.name + ' device.'
    
  ; slf - added HELP keyword and function
  if keyword_set(help) and !d.name eq 'X' then begin
